...
Крайне заметно, что многопоточная программа работает куда быстрее обычной.
При этом уже с 2-х потоков виден прирост практически в 2 раза.
Однако далее он становится все незначительнее. 
...
При этом заметна общая тенденция: максимальная эффективность достигается при 6-ти потоках:
...
\begin{tikzpicture}
    ...
    \addplot table [x index = 0, y index = 3, col sep=comma] {data/base.csv};
    \addplot table [x index = 0, y index = 3, col sep=comma] {data/opt.csv};
    \legend{Без оптимизаций, С оптимизациями}
    \end{axis}
\end{tikzpicture}
